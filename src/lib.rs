...
#[inline]
fn from_base58(bytes: &[u8]) -> Result<Vec<u8>, FromBase58Error> {
    let mut bin = [0u8; 132];
        ...
        let zcount = bytes.iter().take_while(|&&x| x == b'1').count();
        let mut i = zcount;
        let b58: Vec<u8> = bytes.iter().copied().collect();
